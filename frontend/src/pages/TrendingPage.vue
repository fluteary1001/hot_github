  ...
  <div class="trending-page">
    ...
    <div class="page-header">
      <div class="header-info">
        ...
        <p class="page-desc">发现今日最热门的开源项目</p>
      </div>
      <div class="header-actions">
        <el-select v-model="since" placeholder="时间范围" class="since-select">
          <el-option label="今日" value="daily" />
          <el-option label="本周" value="weekly" />
          <el-option label="本月" value="monthly" />
        </el-select>
        <el-button :icon="Refresh" @click="loadTrending" class="refresh-btn">刷新</el-button>
      </div>
    </div>

    <!-- Trending Grid -->
    <div class="trending-grid" v-loading="loading">
      <div
        v-for="(project, index) in trendingList"
      ...
      >
        ...
        <div class="card-content">
          ...
          <div class="card-meta">
            ...
            <div class="meta-item" v-if="project.stars_today">
              <el-icon class="meta-icon fire"><Sunny /></el-icon>
              <span class="meta-value hot">+{{ formatNumber(project.stars_today) }}</span>
            </div>
      ...
      <el-empty v-if="!loading && trendingList.length === 0" description="暂无热点数据" />
    </div>
...
import { TrendCharts, Refresh, Star, Sunny, Download, Link } from '@element-plus/icons-vue'
...
const trendingList = ref<Project[]>([])
...
async function loadTrending() {
  loading.value = true
  try {
    const res = await trendingApi.getOfficial(since.value)
